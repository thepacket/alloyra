import { DECISIONS, DECISION_DRAFT, decisionCandidateName, type DecisionRecord } from "./decisions";
...
const titles: Record<string, string> = {
  "alloyra.decisionDraft.v1": "Unrecorded decision draft",
...
export function studyReport(study: SavedStudy): string {
  ...
  const decisions=JSON.parse(study.slices[DECISIONS]??"[]") as DecisionRecord[];
  const decisionHtml=decisions.length?`<h2>Recorded decisions</h2>${decisions.map((r)=>`<article><h3>${escapeHtml(r.outcome)}</h3><p>Reviewer: ${escapeHtml(r.reviewer)} · ${escapeHtml(r.recordedAt)}</p><p>${r.selected?escapeHtml(decisionCandidateName(r.snapshot,r.selected)):"No candidate selected"}</p><p>${escapeHtml(r.rationale)}</p><p><strong>Unresolved:</strong> ${escapeHtml(r.unresolved)}</p><ul>${r.alternatives.map((a)=>`<li>${escapeHtml(decisionCandidateName(r.snapshot,a.candidate))}: ${escapeHtml(a.reason)}</li>`).join("")}</ul><details><summary>Evidence frozen with this decision</summary>${render(r.snapshot)}</details></article>`).join("")}`:"";
...
${decisionHtml}${comparisonTable}<h2>Shortlist</h2><ul>${shortlist || "<li>No candidates selected</li>"}</ul>
${Object.entries(study.slices).filter(([key])=>key!==DECISIONS).map(([key, raw]) => `<details><summary>${escapeHtml(titles[key] ?? key)}</summary>${render(JSON.parse(raw))}</details>`).join("")}
